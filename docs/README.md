...
│   └── elevation_loader.gd    # Загрузчик высот (SRTM30m через OpenTopoData API)
├── textures/
...
- **Дороги** - полосы разной ширины (1.5-12 м) с текстурами разметки, следуют рельефу
...
- **Рельеф** - SRTM30m данные, все поверхности следуют реальной высоте (см. [ELEVATION.md](ELEVATION.md))
...
- **[ELEVATION.md](ELEVATION.md)** - Система рельефа
  - SRTM30m данные через OpenTopoData API
  - 10x10 грид на чанк, билинейная интерполяция
  - Subdivision полигонов и дорог по 10м сетке
  - Тестирование (elevation flyover test)
