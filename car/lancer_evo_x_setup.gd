...
## Body paint material = "Vehicle_Exterior_mm_ext". The lamp lens is ONE mesh ("lightglass")
## spanning BOTH ends, so it is split at the triangle level into front (warm white) / rear
## (red) emissive sub-meshes by LampEmissive.split_lens_mesh (the real lens glows, no proxy
## boxes). Placed SpotLights/OmniLights remain for road illumination + brake.
...
var _head_mats: Array = []            # front lens materials (glow at night)
var _tail_glow_mats: Array = []       # rear lens materials (glow + brake)
...
func _ready() -> void:
	...
	_setup_taillights()
	_setup_headlights()
	# real lens: split the single spanning "lightglass" mesh into front-white / rear-red
	var r: Dictionary = LampEmissive.split_lens_mesh(self, ["lightglass"], 0.0, 0.7, true, false)
	_head_mats = r["head"]
