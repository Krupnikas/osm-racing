...
## Player (GEVP) setup for the Ford Focus ST 2006.
##
## Pipeline-generated per-car setup script (see docs/CAR_INTEGRATION_PIPELINE.md).
## The model is a material-grouped Sketchfab GLB: all four wheels live in single
## merged Tire/Rim/BrakeDisc/Caliper meshes, so CarWheelRig splits them into four
## recentred, spinnable containers which are then reparented onto the GEVP wheel
## nodes (Matiz-style) so the real model wheels spin + steer + travel with the
## suspension.
##
## Model native forward = +Z. In the GEVP scene the model node is rotated 180°
## (forward = -Z) and scaled ~100. Front wheels therefore sit at vehicle -Z.

const CarWheelRig := preload("res://tools/car_wheel_rig.gd")

# Official Ford Focus ST colours (from new_cars_color_mapping.md). Player default
# is Performance Blue; full list exposed for future showroom colour selection.
const BODY_COLORS := {
	"electric_orange": Color(0.851, 0.416, 0.106),
	"performance_blue": Color(0.149, 0.400, 0.663),
	"panther_black": Color(0.031, 0.035, 0.039),
	"moondust_silver": Color(0.722, 0.729, 0.722),
	"sea_grey": Color(0.333, 0.361, 0.376),
	"frozen_white": Color(0.949, 0.945, 0.917),
	"colorado_red": Color(0.655, 0.098, 0.125),
	"aquarius_blue": Color(0.459, 0.659, 0.698),
}
var body_color := BODY_COLORS["performance_blue"]
...
func _ready() -> void:
	...
	if DEBUG_FORWARD:
		_add_forward_arrow()
	print("Ford Focus ST setup complete")
...
func _process(_delta: float) -> void:
	...
	if _night_manager and "is_night" in _night_manager:
		var n: bool = _night_manager.is_night
		if n != _is_night:
			_is_night = n
...
func _rig_wheels() -> void:
	...
	if not rig.get("ok", false):
		push_warning("Focus ST: wheel rig failed: " + str(rig.get("note", "")))
		return
	if not _vehicle or not _vehicle.get("front_left_wheel"):
		return
	# GEVP spinning wheel nodes
	var nodes := [
	...
	# Reparent each model wheel container onto the nearest GEVP wheel node, so
	# left/right/front/rear placement is correct regardless of the model's 180°
	# rotation.
...
func _change_body_color() -> void:
	for mesh in _find_all_meshes(self):
		...
		for i in range(sc):
			...
			if mn == "paint" and mat is StandardMaterial3D:
...
func _setup_taillights() -> void:
	...
	for x in [-0.6, 0.6]:
		...
		l.name = "BrakeLight"
		l.position = Vector3(x, 0.85, 1.95)
...
func _setup_headlights() -> void:
	...
	for x in [-0.62, 0.62]:
		var l := SpotLight3D.new()
		l.name = "Headlight"
		l.position = Vector3(x, 0.72, -1.85)
...
func _add_forward_arrow() -> void:
	...
	cyl.height = 1.5
	arrow.mesh = cyl
	arrow.rotation_degrees = Vector3(90, 0, 0)  # lie along Z
	arrow.position = Vector3(0, 1.2, -1.6)       # vehicle forward = -Z
