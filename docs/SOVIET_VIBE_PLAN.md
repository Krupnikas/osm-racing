...
**Status:** Phases 0–5, M (day/night/rain), S (storefronts) shipped; **Phase 6 vegetation now a full L1–L5 system** (2026-06-12). Remaining: Phase G graffiti (blocked on user textures), procedural-clutter litter/deform polish, Phase 7 final A/B montage. See §11 log + §15.
...
- **2026-06-05 — Phase 4 DONE (sidewalk kerbs / поребрик).** Re-opened after the user clarified the goal: road curbs already work (terrain-polygon edge curb between road↔grass); what was missing is a kerb on the **sidewalks** themselves. The 8 prior failures ([[curb_junction_debug]]) fought footway×footway junctions with a polyline approach. New approach (`osm_terrain_generator.gd` `_union_footway_polys` + `_build_sidewalk_kerbs`, gated `enable_sidewalk_curbs`): footways are polygons, so **union them per chunk** → kerb traces only the outer perimeter (no kerb through junction middles), **suppress edges whose outside is a vehicle road** (`_is_point_on_vehicle_road_neighborhood`, so no kerb where the sidewalk directly abuts a road), and **subdivide each edge ~3 m sampling elevation** so the kerb follows terrain (fixes long 2-point footways whose straight chord sank under humps and vanished). Low ~4 cm concrete lip (`_curb_material`). Verified red/blue/green debug at the junction `59.150211/37.949307`; shots `screenshots/curb_sw_redblue_wide.png`, `curb_sw_fixed_noveg.png`, `curb_sw_low_closeup.png`. Full write-up: [curb_phase4_debug.md](curb_phase4_debug.md).
  - **4b (concrete sidewalk) — SKIPPED** by user direction: modern Russian тротуары are asphalt, so the existing Asphalt022 sidewalk texture is correct.
  - **4c (verge) — weedier grass only** (no fence, per user): `shaders/ground.gdshader` adds multi-scale world-space "разнотравье" clumps (richer/darker green patches over the dry base) so verges read as an unkempt lawn instead of flat tiled yellow. Shot `screenshots/curb_sw_weedy.png`.
- **Remaining (optional polish):** Phase 7 sky cumulus clouds (day sky is the `day_clear_2k.hdr` panorama — cloudless; swap risks sun/exposure mismatch since it now also drives ambient) + final A/B montage. *(Phase 6 vegetation density is now a full L1–L5 system — see §15.)*
...
- **2026-06-05 — Distant skyline haze DONE.** Far apartment blocks now dissolve into warm atmospheric haze (depth + scale), foreground stays crisp. Kept the existing **exponential** fog (mode-safe) rather than depth fog — depth-mode fog is multiplied by `fog_density`, so the density-based graphics_settings + night/rain systems would have broken it (would've required a 3-file rework). Instead bumped the day fog: `fog_density 0.0002→0.0015`, `fog_aerial_perspective 0.35→0.6` in three places that must agree — generator runtime override ([osm_terrain_generator.gd:19432](../osm/osm_terrain_generator.gd#L19432)), `graphics_settings._apply_render_distance` (`0.4→0.9 / render_distance`, aerial 0.6), and `main.tscn` Environment. Night (sets 0.003 absolute) and rain (×1.5 → 0.00225) stay correct and read *denser*, which is right for those modes. Verified all three: [skyline_after](../screenshots/skyline_after.png) day, [skyline_night](../screenshots/skyline_night.png), [skyline_rain](../screenshots/skyline_rain.png).
...
- **2026-06-12 — Phase 6 vegetation: full L1–L5 system DONE** (commit `5b9d480`, branch `soviet-vibe-visuals`). Layered roadside greenery replacing the flat-verge / empty-mid-ground gap: **L1** street-line birch rows (road-walk, primary/secondary/tertiary + confident residential, march outward past the sidewalk onto the grass verge, capped near-road colliders); **L2** verge shrub clusters + **L2b** polygon shrub scatter into parks/courtyards/industrial interiors; **L3** nettle patches in industrial/brownfield (solid 3D `nettle_2` model — the first leaf-on-black billboard was abandoned); **L4** dandelion/clover flower patches on kept-green land; **L5** run-over flattening (flowers + nettles press flat as a visible imprint under a wheel, persistent). All per-chunk MultiMesh + LOD, deterministic, gated, ~115–120 FPS held. Fixed `_is_point_on_footway` to use the 2× **rendered** footway width (sidewalks draw at width×2) so nothing lands on pavement. Full doc: [VEGETATION_PLACEMENT_PLAN.md](VEGETATION_PLACEMENT_PLAN.md).

## 15. Remaining work (2026-06-12 revision)

The atmosphere/light/material/streetscape core (Phases 0–5), all three modes (Phase M), storefronts (Phase S), and now the full vegetation system (Phase 6) are shipped. What's left is **polish + two parked features**:

1. **Phase G — graffiti on walls** 📋 *blocked on the user* — design is agreed (§14); needs the user to drop transparent-PNG tags into `decorations/russia/cherepovets/graffiti-atoms/`, then it's a quick build.
2. **Procedural clutter — Phase 3-4 tail** ([PROCEDURAL_CLUTTER_PLAN.md](PROCEDURAL_CLUTTER_PLAN.md)): trash bags shipped; **parked** = crumpled paper near schools + the deform polish (dent shader for bins, squash for cones/bags) + remaining SFX.
3. **Phase M5 — denser daytime traffic** — the one day-life cue not yet pushed (storefront signage, distant skyline haze, wheel-track polish are all done).
4. **Phase 7 — final A/B montage** across day/night/rain to formally close the Definition of Done (§8/§12.4). Sky cumulus stays deprioritized (HDRI ambient risk).
5. **Optional vegetation follow-ups** (own doc): extend run-over flatten to small shrubs; clean up the dormant luminance-cutout code + old `nettle.glb`; verge density tuning.

None of these are blockers — the world already reads as the intended provincial-Soviet place in all three modes. Items 1 & 2 are the most "content-complete-feeling" additions; item 4 is the closing audit.
